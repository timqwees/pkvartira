...
class Article extends Network
{
    ...
    {
        try {
            return Database::send("SELECT COUNT(id) as count FROM {$this->table_name}")[0]['count'];
        } catch (\Exception $e) {
            return false;
        }
    }

    public function getPaginatedArticles(int $current_page, int $count_show_article_current_page)
    {
        try {
            $offset = ($current_page - 1) * $count_show_article_current_page;
            $result = Database::send(
                "SELECT * FROM {$this->table_name} ORDER BY created_at DESC LIMIT ? OFFSET ?",
                [$count_show_article_current_page, $offset]
            );
            return $result;
        } catch (\PDOException $e) {
            return false;
    ...
    {
        try {
            $result = Database::send("SELECT DISTINCT category FROM {$this->table_name}");
            return $result;
        } catch (\PDOException $e) {
            return false;
        }
    ...
    {
        try {
            $result = Database::send("SELECT DISTINCT * FROM {$this->table_name} ORDER BY created_at DESC LIMIT 5");
            return $result;
        } catch (\Exception $e) {
            return false;
        }
